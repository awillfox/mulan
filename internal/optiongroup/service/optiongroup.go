...
func (s *Service) SetMenuGroups(ctx context.Context, menuID int32, groupIDs []int32) error {
	if err := s.q.ClearMenuOptionGroups(ctx, menuID); err != nil {
		return fmt.Errorf("clear: %w", err)
	}
	for i, gid := range groupIDs {
		if err := s.q.AttachMenuOptionGroup(ctx, sqlc.AttachMenuOptionGroupParams{
			MenuID:        menuID,
			OptionGroupID: gid,
			SortOrder:     int32(i),
		}); err != nil {
...
func (s *Service) GroupsForMenus(ctx context.Context, menuIDs []int32) (map[int32][]GroupWithOptions, error) {
	...
	allGroups, err := s.q.ListOptionGroups(ctx)
	if err != nil {
		return nil, fmt.Errorf("list groups: %w", err)
	}
	byID := make(map[int32]sqlc.OptionGroup, len(allGroups))
...
func (s *Service) ResolveOptions(ctx context.Context, optionIDs []int32) ([]sqlc.Option, error) {
	if len(optionIDs) == 0 {
		return nil, nil
	}
	return s.q.GetOptionsByIDs(ctx, optionIDs)
}
