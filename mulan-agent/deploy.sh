...
#
# Prereq (one-time):
#   - SSH key auth working: `ssh coffee@192.168.1.52 whoami` returns without prompt.
...
if HTTP=$(curl -sS -m 5 -o /dev/null -w '%{http_code}' "http://192.168.1.52:8090/pos" 2>/dev/null) && [[ "$HTTP" == "200" ]]; then
    ok "http://192.168.1.52:8090/pos → 200"
else
